...
class Patient extends MY_Controller {
	...
	public function details($patient_id) {
		$this->load->model(['patient_sample_model', 'gazetteer_model','country_model','vaccine_model']);
    ...
    public function search($patient_code) {
            ...
            if($patient == null){
                ...
                if($patient == null){
                    ...
                    if($patient !== null){
                        ...
                        $is_patient_child = false;
                        $is_camlis_patient = true;
                        //reset pid column
                        $patient['pid'] = $original_patient_code; // 08-07-2021
                    }
                }else{
        ...
        $data['result'] = json_encode(['patient' => $patient , 'is_patient_child' => $is_patient_child , 'is_camlis_patient' => $is_camlis_patient]);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isPMRSPatientID($patient_code)) {
                ...
                if ($patient_info && !empty($patient_info['pid'])) {
                    ...
                    'residence','nationality','country','date_arrival','passport_number','seat_number','is_positive_covid','test_date',
                    'is_contacted','contact_with','relationship_with_case','travel_in_past_30_days','flight_number','is_direct_contact',
                    'country_name','vaccination_status','first_vaccinated_date','vaccine_id','occupation','second_vaccinated_date','third_vaccinated_date','second_vaccine_id' , 'forth_vaccinated_date','third_vaccine_id' ], $patient);
    ...
    public function update_outside_patient($pid) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $patient = elements(['patient_name', 'sex', 'dob', 'phone', 'province', 'district', 'commune', 'village',
                    'residence','nationality','country','date_arrival','passport_number','seat_number','flight_number',
                    'is_positive_covid','test_date','is_contacted','contact_with','relationship_with_case','travel_in_past_30_days',
                    'is_direct_contact','country_name','vaccination_status','first_vaccinated_date','vaccine_id','occupation','second_vaccinated_date', 'second_vaccine_id','third_vaccinated_date', 'third_vaccine_id','forth_vaccinated_date'], $patient);                    
    ...
    public function merge() {
        ...
        if ($this->form_validation->run() === TRUE && !isPMRSPatientID($patient_id_source) ) {
            ...
            if ($source_patient && $destination_patient) {
                $source_patient_samples = $this->patient_sample_model->get_patient_sample(NULL, NULL, strval($source_patient['pid']));
    ...
    */
    public function upload_file(){
        ...
        if($this->upload->do_upload("theExcelFile")){
            ...
            foreach ($cell_collection as $cell) {
                ...
                $patient_code  = $objPHPExcel->getActiveSheet()->getCell("A".$row)->getValue(); 
                //The header will/should be in row 1 only. 
                if ($row == 1 || $row == 2) {
                    $header[$row][$column] = $data_value;
                } else {
                    $dt[$row][$column] = $patient_name;
                    if($patient_name !== null || $patient_code !== null){
    ...
    public function add_line_list_full_form(){
        ...
        $col = array(
            ...
            "forth_vaccinated_date"    => 64,
            "third_vaccine_name"      => 65,
            "occupation"                      => 66,
        ...
        foreach($data as $key => $row){
            ...
            if(!empty($row[$col["patient_name"]])){
                if( isset($row[$col["patient_name"]]) && 
                    ...
                    $isCamlisPatient            = (empty($row[$col["is_camlis_patient"]]) || $row[$col["is_camlis_patient"]] == null || $row[$col["is_camlis_patient"]] == 0) ? FALSE : TRUE; //22062021
                    ...
                    if ($isPMRSPatientID) {
                        ...
                        //Check Patient Code existent                        
                        $patient_info        = $this->patient_model->get_outside_patient(FALSE, $patient_code);
                        //in case it is camlis_patient                        
                        if(!$patient_info) {
                            if($original_patient_code !== ""){
                        ...
                        if ($patient_info && !empty($patient_info['pid'])) {
                            ...
                            $isPatientExist         = true;
                            $is_patient_child       = (empty($patient_info['parent_code']) || $patient_info['parent_code'] == null) ? false : true; //added 29-06-2021
                        }
                        ...
                        /** 14072021 */
                        $vaccination_status = ($row[$col["vaccination_status_code"]] !== "") ? $row[$col["vaccination_status_code"]] : null;
                        $occuation          = null;
                        $vaccine_id         = null;
                        $second_vaccine_id = null;
                        $first_vaccinated_date = null;
                        $second_vaccinated_date = null;
                        $third_vaccinated_date = null;
                        $forth_vaccinated_date = null;
                        $third_vaccine_id = null;
                        
                        if($vaccination_status !== null){
                            $occuation          = ($row[$col["occupation"]] !== "") ? $row[$col["occupation"]] : null;
                            $vaccine_id         = ($row[$col["vaccine_id"]] !== "") ? $row[$col["vaccine_id"]] : null;
                            $second_vaccine_id  = ($row[$col["second_vaccine_id"]] !== "") ? $row[$col["second_vaccine_id"]] : null;
                            $first_vaccinated_date    = ($row[$col["first_vaccinated_date"]] !== "") ? date('Y-m-d', strtotime($row[$col["first_vaccinated_date"]])): null;
                            $second_vaccinated_date    = ($row[$col["second_vaccinated_date"]] !== "") ? date('Y-m-d', strtotime($row[$col["second_vaccinated_date"]])): null;
                            $third_vaccinated_date  = ($row[$col["third_vaccinated_date"]] !== "") ? date('Y-m-d', strtotime($row[$col["third_vaccinated_date"]])): null;
                            $forth_vaccinated_date  = ($row[$col["forth_vaccinated_date"]] !== "") ? date('Y-m-d', strtotime($row[$col["forth_vaccinated_date"]])): null;//09022022
                            $third_vaccine_id  = ($row[$col["third_vaccine_id"]] !== "") ? $row[$col["third_vaccine_id"]] : null; //09022022
                        }
                        /**End */
                        ...
                        $patient = array(
                            ...
                            'first_vaccinated_date'     => $first_vaccinated_date,
                            'second_vaccinated_date'    => $second_vaccinated_date,
                            'occupation'                => $occuation,
                            'third_vaccinated_date'     => $third_vaccinated_date,
                            'second_vaccine_id'         => $second_vaccine_id,
                            'forth_vaccinated_date'     => $forth_vaccinated_date, //09022022
                            'third_vaccine_id'          => $third_vaccine_id//09022022
                        );

                        if($isPatientExist == true){

                            if($is_patient_child == false){
                                if ($this->patient_model->update_outside_patient($pid, $patient)) {
                                ...
                                // Get information from parent and update
                                $parent_patient_info = $this->patient_model->get_parent_patient($original_patient_code);
                                if($parent_patient_info){
                                    $parent_patient_info['parent_code'] = $original_patient_code;
                                    if ($this->patient_model->update_outside_patient($pid, $parent_patient_info)) {
                                        $patient["patient_code"] = $original_patient_code;
                                        $patient["msg"]         = _t('global.msg.save_success');
                                        $patient["pstatus"]     = true;
                                    }else{
                                        $patient["msg"]         =  _t('global.msg.save_fail');
                            ...
                            if ($isCamlisPatient){
                                $patient['patient_code'] = $original_patient_code;
                                $patient['parent_code']  = $original_patient_code;
                            }
                            ...
                            if($psample_id > 0 || $patient["sample_status"]){
                                ...
                                if(isset($row[$col["test_name"]])){
                                    if($row[$col["test_id"]] !== ""){
                                        ...
                                        }else if($row[$col["test_id"]] == 509){
                                            $sample_tests = array(509); // id table patient_sample_tests
                                            $sample_details = array(
                                                "department_sample_id"  => 29, 
                                                "sample_description"    => "-1", 
                                                "first_weight"          => null, 
        ...
        $this->session->set_userdata('timestamp', time()); //reset timestamp
