...
<body>
    <input type="hidden" name="verify" id = "verify" value="<?php if(!empty($patient_sample['verify'])) echo $patient_sample['verify']; ?>">
    <input type="hidden" name="patient_sample_id" id = "patient_sample_id" value="<?php echo $patient_sample['patient_sample_id']; ?>"> 
    <?php 
    ...
    <page size="A4">       
        <table border="0" width="100%">
                    ...
                    <th>
                        <table width="100%" border="0" class="patient-info">
                            ...
                            <tr>
                                ...
                                <td class="text-no-wrap" data-gender = "<?php echo $patient_info['sex'];?>">
                                    <?php echo (trim($patient_info['sex']) == 'M' || trim($patient_info['sex']) == '1')  ? _t('male') : _t('female'); ?>
                                    &nbsp;&nbsp;
                                    <?php
                                        echo '<b>'._t('age').' : </b>';
                                        $age = calculateAge($patient_info['dob'], $patient_sample['collected_date']);
                                    ?>
                                    <span class="age-year" data-age="<?php echo $patient_info['dob'];?>/<?php echo $patient_sample['collected_date']; ?>"></span> <?php echo $age->y.' '._t('global.year') ?> &nbsp;
                            ...
                            <tr>
                                ...
                                <td><?php
                                $diagnosis = !empty($patient_sample['clinical_history'])? trim($patient_sample['clinical_history']) : null;
                                if(isset($diagnosis) && strlen($diagnosis) > 0){
                                    echo $diagnosis;
                                }else{
                                    //echo $patient_sample['for_research'];
                                    if($patient_sample['for_research'] > 0){
                                        echo $FOR_RESEARCH_ARR[$patient_sample['for_research']];
                                    }
                                }
                                 ?>
            ...
            <tbody class="sample-result">
                    ...
                    <td>
                        <table width="100%" border="0" class="test-result">
                            ...
                            <?php foreach ($sample->tests as $test) {
                                ...
                                if ( !empty($test_result['result']) &&  is_array($test_result['result']) && count($test_result['result']) > 0 && !$test->is_rejected && !$test->is_heading) {
                                    $i = 1;
                                    $test_result['result'] = array_values($test_result['result']);

                                    foreach ($test_result['result'] as $organismIndex => $resultItem) {
                                        $machine_name = (!empty($resultItem['machine_name'])) ? " (".$resultItem['machine_name'].")" : "" ;
                                        //Show Test info for first result only                                        
                                        $testName  = $i == 1 ? "<p class='value-pair'><span class='name' data-machine_name='here'>".$indents.$test->test_name.$machine_name. "</span></p>" : "";
                                        $unit_sign = "";
                                        $ref_range = "";
                                    ...
                                    if ($test->is_heading) {
                                        ...
                                        $ref_range_text = "";
                                        $machine_name = (!empty($patient_sample_results[$test->patient_test_id]['machine_name'])) ? " (".$patient_sample_results[$test->patient_test_id]['machine_name'].")" : "" ;
                                        $result_str = ''; //dev
                                        ...
                                        if (is_array($ref_range)) {
                                            ...
                                            /* 
                                             * 02052022 
                                             * Feedback from Santuk Kompong patient_sample id = 3859397
                                             * 
                                             * */
                                            if(isset($ref_range['range_sign'])){
                                                $ref_range['range_sign'] = trim($ref_range['range_sign']); // remove space 
                                            }
                                        ...
                                        echo "<tr>";
                                        echo "<td style='padding-left: ".$padding."px'><p class='value-pair' data-machine_name='".$machine_name."'>";
                                        echo "<span class='name'>".$test->test_name.$machine_name."</span>";
                                        echo "</p></td>";
                                        echo "<td>".$rowValue."</td>";
                ...
                <!-- Comment -->
                <?php if (!empty($result_comment[$sample->department_sample_id]['result_comment']) || (!empty($result_comment[$sample->department_sample_id]['reject_comment'])) || (!empty($patient_sample['reject_comment']) && $total_department_sample == $department_sample_count)): ?>
                    <tr>
                        <td colspan="4">
                            <table>
                                <tr>
                                    ...
                                    <td style="padding-left: 6px">
                                        <?php
                                            ...
                                            /*
                                            * 29/08/2018
                                            * Add new variable $comment_result
                                            */
                                            $comment_reject = (!empty($result_comment[$sample->department_sample_id]['reject_comment'])) ? $result_comment[$sample->department_sample_id]['reject_comment'] : "";
                                            /*
                                            * 29/08/2018
                                            * $comments
                                            * for combine result and reject comment
                                            */
                                            $comments = $comment_result . $comment_reject;
                                            /*
                                            * Old code
            ...
            <tr>
                <td class="text-center text-no-wrap" style="width: 7cm">
                    <?php echo _t('last_test_date')." : ";
                        $test_date = !empty($patient_sample['test_date']) ? DateTime::createFromFormat('Y-m-d', $patient_sample['test_date']) : false;
                ...
                <td class="text-center text-no-wrap" style="width: 7cm">
                    <?php echo _t('report_date')." : ";
                        $printedDate = !empty($patient_sample['printedDate']) ? DateTime::createFromFormat('Y-m-d H:i:s', $patient_sample['printedDate']) : false;
