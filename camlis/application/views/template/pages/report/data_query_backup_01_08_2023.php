        ...
        <div class="col-sm-5 col-md-4">
            <form id="frm-condition">
                <table id="tbl-condition">
					...
					<tr class="header">
						<th></th>
						<th class="text-blue" colspan="2"><h4>Enter Primary Condition</h4></th>
					</tr>

					<tr class="query-field labo-wrapper">
						<td class="text-right">Period Filter <span class="text-danger">*</span></td>
						<td colspan="2">
							<select id="filter-period" onchange="periodFilter()">
								<option value="1">Collected Date</option>
								<option value="2">Received Date</option>
								<option value="3">Tested Date</option>
							</select>
						</td>
					</tr>

					<tr class="query-field" id="primary-period-filter-tr" style="display: none">
						<td class="text-right title" id="primary-period-filter-title">-- Date</td>
						<td colspan="2">
							<div class="input-group">
								<input type="text" class="form-control input-sm dtpicker" id="date-min" name="collected_date[min]">
								<span class="input-group-addon"><?php echo _t('data_query.to'); ?></span>
								<input type="text" class="form-control input-sm dtpicker" id="date-max" name="collected_date[max]">
							</div>
						</td>
					</tr>
                    ...
                    <tr class="query-field">
                        ...
                        <td class="text-center"><input type="checkbox" value="1" class="show-field" data-name="collected_date" name="collected_date[is_show]" checked></td>
                    </tr>
                    <!-- Diagnosis -->
					<tr class="query-field hide">
                    ...
                    <tr class="query-field">
                        ...
                        <td class="text-center"><input type="checkbox" value="1" class="show-field" data-name="received_date" name="received_date[is_show]" checked></td>
                    </tr>
                    <!-- Sample Source -->
                    <tr class="query-field">
...
<script>
	function periodFilter() {
		if($('#filter-period').val()>0){
			$('#primary-period-filter-tr').show();
			if($('#filter-period').val()=='1'){
				$('#collection-date-min').prop('disabled', true)
				$('#collection-date-max').prop('disabled', true)

				$('#received-date-min').prop('disabled', false)
				$('#received-date-max').prop('disabled', false)

				$('#test-date-min').prop('disabled', false)
				$('#test-date-max').prop('disabled', false)
			}
			if($('#filter-period').val()=='2'){
				$('#collection-date-min').prop('disabled', false)
				$('#collection-date-max').prop('disabled', false)

				$('#received-date-min').prop('disabled', true)
				$('#received-date-max').prop('disabled', true)

				$('#test-date-min').prop('disabled', false)
				$('#test-date-max').prop('disabled', false)
			}
			if($('#filter-period').val()=='3'){
				$('#collection-date-min').prop('disabled', false)
				$('#collection-date-max').prop('disabled', false)
				...
				$('#test-date-min').prop('disabled', true)
				$('#test-date-max').prop('disabled', true)
			}
		}
		$('#primary-period-filter-title').html($('#filter-period option:selected').text()+' <span class="text-danger">*</span>')
	}
</script>
