      ...
      $(this).html('Please wait ...').attr('disabled', 'disabled'); // prevent multiple click      
        ...
        if (province_id == ""){
          ...
          setTimeout(function(){
            ...
            $("#btnPrint").html('Print').removeAttr('disabled'); // prevent multiple click
            btnPrint
          },300)
          return false;
        }
          ...
          success: function (resText) {
            ...
            if(resText.status == true){
              ...
              if(data.user[0]){
                htmlStr += '<td>ឈ្មោះ</td>';
                htmlStr += '<td>:</td>';
                htmlStr += '<td>'+data.user[0].fullname+'</td>';
                htmlStr += '</tr>';
                htmlStr += '<tr>';
                htmlStr += '<td>ខេត្ត ក្រុង</td>';
                htmlStr += '<td>:</td>';
                htmlStr += '<td>'+data.user[0].province_name+'</td>';
                htmlStr += '</tr>';
                htmlStr += '<tr>';
                htmlStr += '<td>ទីតាំង</td>';
                htmlStr += '<td>:</td>';
                htmlStr += '<td>'+data.user[0].location+'</td>';
                htmlStr += '</tr>';
                htmlStr += '<tr>';
                htmlStr += '<td>លេខទូរសព្ទ័</td>';
                htmlStr += '<td>:</td>';
                htmlStr += '<td>'+data.user[0].phone+'</td>';
                htmlStr += '</tr>';
                htmlStr += '<tr>';
                htmlStr += '<td>អ៊ីម៉េល</td>';
                htmlStr += '<td>:</td>';
                htmlStr += '<td>'+data.user[0].email+'</td>';
                htmlStr += '</tr>';
              }
              
            }else{
              htmlStr += '<tr>';
              htmlStr += '<td>ពុំមានលទ្ធផលលេខកូដនេះទេ</td>';
              htmlStr += '</tr>';
            }
            setTimeout(function(){
