        ...
        $sql = "
            ...
            CASE
                ...
                FROM
                    ...
                    FROM
                        ...
                        WHERE
                            TO_CHAR( ( concat ( pv.received_date, ' ', pv.received_time ) ) :: TIMESTAMP, 'YYYY-mm-dd HH24:MM' ) >= '".$startDateTime."'::timestamp
                            AND TO_CHAR( ( concat ( pv.received_date, ' ', pv.received_time ) ) :: TIMESTAMP, 'YYYY-mm-dd HH24:MM' ) <= '".$endDateTime."'::timestamp
                            
                            /*TO_CHAR(timestamp concat( pv.received_date, ' ', pv.received_time ), 'YYYY-mm-dd HH24:MM' ) >= '".$startDateTime."' */
                            /*AND TO_CHAR(timestamp concat( pv.received_date, ' ', pv.received_time ), 'YYYY-mm-dd HH24:MM' ) <= '".$endDateTime."' */
                            /*
                            concat( pv.received_date,pv.received_time) >= '".$startDateTime."::date"."'
                            AND concat( pv.received_date, pv.received_time) <= '".$endDateTime."::date"."'
                            */
            ...
            /* aging section */
                    ...
                    FROM
                        ...
                        WHERE
                            TO_CHAR( ( concat ( pv.received_date, ' ', pv.received_time ) ) :: TIMESTAMP, 'YYYY-mm-dd HH24:MM' ) >= '".$startDateTime."' 
                            AND TO_CHAR( ( concat ( pv.received_date, ' ', pv.received_time ) ) :: TIMESTAMP, 'YYYY-mm-dd HH24:MM' ) <= '".$endDateTime."' 
                            AND pv.\"labID\" = '".$this->session->userdata('laboratory')->labID."' 
                            AND P._day <= 30 
                            AND P._month = 0 
		...
		$sql = " 
                ...
                FROM 
                  ( SELECT
                        ...
                        s.sample_name,
                        lss.source_name,
                        to_char ( stv.test_date, 'YYYY-mm-dd' ) AS test_date,
                        stv.test_name,
                        pr.organism_name AS results,
                        pr.quantity,
                        to_char ( stv.collected_date, 'YYYY-mm-dd' ) AS collected_date,
                        stv.diagnosis,
                        stv.description,
                        stv.sample_volume1,
                        stv.sample_volume2,
