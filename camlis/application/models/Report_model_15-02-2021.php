...
/*		$con=" WHERE DATE_FORMAT(pv.`received_date`,'%Y-%m-%d') >= '".$obj->start."'
    ...
    public function get_raw_data($data) {        
        ...
        if ($has_department || $has_sample_type || $has_sample_description || $has_test) {
            ...
            $this->db->join('camlis_std_sample_test AS std_sampletest', 'psample_test.sample_test_id = std_sampletest."ID"', 'inner');
        ...
        if ($has_test_date || $has_organism || $has_antibiotic || $has_sensitivity ) {
            ...
            $this->db->join('camlis_std_test_organism AS test_organism', 'ptest_result.type = 1 AND ptest_result.result = cast(test_organism."ID" as varchar)', 'left');
        ...
        if ($has_test_date || $has_organism || $has_antibiotic || $has_sensitivity ) {
            $this->db->group_by('ptest_result.test_date , ptest_result.type');
        }
        if ($has_department || $has_sample_type || $has_sample_description || $has_test) {
            $this->db->group_by('std_sampletest.group_result');
        }
