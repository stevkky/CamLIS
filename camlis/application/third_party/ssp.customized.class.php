...
class SSP {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            ...
            for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['orderable'] == 'true' ) {
                    ...
                    $orderBy[] = ($isJoin) ? $col.' '.$dir : '"'.$col.'" '.$dir;
                }
            }
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $columnName = $column['db'];
                    //if($columnName == "ID") { $columnName = '"'.$column['db'].'"'; }

                    if (strpos($column['db'], ".") !== false) {
                        $split = explode(".", $column['db']);
                        $columnName = '"'.$split[0].'"."'.$split[1].'"';
                    }
                    if (isset($column['type']) && 
                        ($column['type'] === 'Number' || $column['type'] === 'Boolean' || $column['type'] === 'bigint') ) {
                        $columnName = '('.$columnName.')::TEXT';
                    }
                    $binding = SSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
                    
                    $globalSearch[] = ($isJoin) ? $columnName." LIKE ".$binding : $columnName." LIKE ".$binding;
                }
            }
        }
        ...
        for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
            ...
            if ( $requestColumn['searchable'] == 'true' && $str != '' ) {
                ...
                $binding = SSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
                $columnSearch[] = ($isJoin) ? $columnName." LIKE ".$binding : $columnName." LIKE ".$binding;
            }
        }
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        if($joinQuery){
            $cols = SSP::pluck($columns, 'db', $joinQuery);
            $select = SSP::build_select($cols);
            $query =  "SELECT $select
			 $joinQuery
			 $where
			 $extraWhere
            ...
            $queryFilter = "SELECT COUNT(1) $joinQuery";
        }else{
            $cols = SSP::pluck($columns, 'db');
            $select = SSP::build_select($cols);
            $query =  "SELECT $select
			 FROM $table
			 $where
            ...
            $queryFilter = "SELECT COUNT(1) FROM $table";
        }

        $data = SSP::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $queryFilter .= " $where $extraWhere $groupBy";
        $resFilterLength = SSP::sql_exec( $db, $bindings, $queryFilter);
        if ($groupBy != '') {
            $recordsFiltered = count($resFilterLength);
        } else {
            //$recordsFiltered = $resFilterLength[0][0];
            if(empty($resFilterLength)){$recordsFiltered="['1','2']";}else{ $recordsFiltered = $resFilterLength[0][0]; } // added 18 Jan 2021
        }
        ...
        if(empty($resTotalLength)){$recordsTotal="['1','2']";}else{ $recordsTotal = $resTotalLength[0][0]; } // added 18 Jan 2021
    ...
    static function build_select($cols) {
        $select = '';
        foreach ($cols as $col) {
            if (strpos($col, "string_agg") !== false || strpos($col, "SUM") !== false 
                || strpos($col, "CASE") !== false || strpos($col, "MAX") !== false) {
                $select .= $col.', ';
    ...
    {
        try {
            SSP::$db_psql = $sql_details['port'] === 5432;
            if (SSP::$db_psql == true) {
                $sql_details['host'] = $sql_details['host'] == null? 'localhost': $sql_details['host'];
                $db = @new PDO("pgsql:host={$sql_details['host']};dbname={$sql_details['db']};port={$sql_details['port']}", 
                    $sql_details['user'],
                    $sql_details['pass']
                );
