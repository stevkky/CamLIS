...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		$query = $this->aauth_db->where('banned', true);
		...
		$query = null;
		$query = $this->aauth_db->where($db_identifier, $identifier);
		$query = $this->aauth_db->where('banned', FALSE);
		...
		if ( $query->num_rows() != 0 && $this->verify_password($password, $row->pass) ) {
            ...
            $role_id = $this->aauth_db->get_where($this->config_vars['user_to_group'],array("user_id"=>$row->id))->row()->group_id;
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
		...
		// banneds
		if (!$include_banneds) {
			$this->aauth_db->where('banned != ', true);
		}
	...
	public function get_perm_id($perm_par) {
		...
		if( is_bool($perm_par) || is_numeric($perm_par) ) { return $perm_par; }
