$(function () {
    ...
    $("#patient-name").autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    console.log(data);
                    // in case user does not select on any result
                    if(data.length > 0) $("#patient-code").val(data[0].patient_code);
                    else $("#patient-code").val('');
        ...
        select: function (event, ui) {
            console.log(ui.item.patient_code);
    ...
    $("#btnGenerate").on("click", function (evt) {
        evt.preventDefault();
        var sample_number  = $("#sample-number").val().trim();
        var patient_code   = $("#patient-code").val().trim();

        /**04042022 */
        if(sample_number === '' && patient_code === ''){
                    ...
                    $.ajax({
                        ...
                        success: function (resText) {
                            ...
                            if(resText.result == false){
                                alert("No Test were assigned to the patient sample id");
                                myDialog.showProgress('hide');
                            }else{
                                console.log(patient_sample_id);
                                console.log(encodeURIComponent(patient_sample_id.join(',')))
                                $.ajax({
                                    ...
                                    error: function() {
                                        myDialog.showDialog('show', { text : 'Something went wrong...!', style : 'warning' });
                                        myDialog.showProgress('hide');
                                    }
                                });
                            }
